...
def write_lidar_data(lidar_data='./utils/lidar_data.txt', data=None):
    print('-lidar_data', lidar_data)
    txt = open(lidar_data, 'w')
...
def demo(data_idx=11, object_idx=0, show_images=False, show_lidar=True, show_lidar_2d=False, show_lidar_box=True, show_project=True, show_lidar_frustum=True):
    import mayavi.mlab as mlab
    from viz_util import draw_lidar, draw_lidar_simple, draw_gt_boxes3d
    dataset = kitti_object(os.path.join(ROOT_DIR, 'dataset/KITTI/object'))
    ...
    print('---uvdepth', uvdepth)
    uvdepth[0, 2] = 200  # some random depth
    print('---uvdepth', uvdepth)   # [[761.565 225.46   20.   ]]
    box2d_center_rect = calib.project_image_to_rect(uvdepth)
    print('---box2d_center_rect', box2d_center_rect)   # [[-0.06467274  0.00050125  0.0002    ]]
                                                       # [[0.38074981 0.12764628 2.        ]]
                                                       # [[ 4.3899537   1.27206606 20.        ]]
    ...
    if show_lidar_box:
        ...
        print('---box3d_pts_3d_velo.shape', box3d_pts_3d_velo.shape)   # ---box3d_pts_3d_velo.shape (8, 3)
        box3droi_pc_velo, _ = extract_pc_in_box3d(pc_velo, box3d_pts_3d_velo)  # get pointcloud_velo in 3d box
        print('--- box3droi_pc_velo.shape',  box3droi_pc_velo.shape)   # (376, 3)
        print(('Number of points in 3d box: ', box3droi_pc_velo.shape[0]))
        # write_lidar_data(lidar_data='./utils/lidar_data1.txt', data=box3droi_pc_velo)
    ...
    # show_lidar_frustum_with_3d = True
    # if show_lidar_frustum_with_3d:
    #     import mayavi.mlab as mlab
    #     # Only display those points that fall into 2d box
    #     print(' -------- LiDAR points in a frustum from a 2D box --------')
    #     xmin,ymin,xmax,ymax = \
    ...
    #
    #     # drwa 2d box in 3d
    #     #uvdepth [[ 2.16261821  0.63627729 10.        ]]
    #     # distance = [1, 5, 10, 15, 20, 30, 50, 70, 80, 100]
    #     distance = [5]
    #     for d in distance:
    #         p_tl = np.array([[xmin, ymin, d]])
    #         p_tr = np.array([[xmax, ymin, d]])
    #         p_dl = np.array([[xmin, ymax, d]])
    #         p_dr = np.array([[xmax, ymax, d]])
    #
    #         print('---p_tl', p_tl)
    #         print('---p_tr', p_tr)
    #         print('---p_dl', p_dl)
    #         print('---p_dr', p_dr)
    #         box2d_p_tl = calib.project_image_to_rect(p_tl)
    #         box2d_p_tr = calib.project_image_to_rect(p_tr)
    #         box2d_p_dl = calib.project_image_to_rect(p_dl)
    #         box2d_p_dr = calib.project_image_to_rect(p_dr)
    #         print('---box2d_p_tl', box2d_p_tl)
    #         print('---box2d_p_tr', box2d_p_tr)
    #         print('---box2d_p_dl', box2d_p_dl)
    #         print('---box2d_p_dr', box2d_p_dr)
    #
    #         box3d_p_tl = [box2d_p_tl[:, 2], -box2d_p_tl[:, 0], box2d_p_tl[:, 1]]
    #         box3d_p_tr = [box2d_p_tr[:, 2], -box2d_p_tr[:, 0], box2d_p_tr[:, 1]]
    #         box3d_p_dl = [box2d_p_dl[:, 2], -box2d_p_dl[:, 0], box2d_p_dl[:, 1]]
    #         box3d_p_dr = [box2d_p_dr[:, 2], -box2d_p_dr[:, 0], box2d_p_dr[:, 1]]
    #
    #         mlab.plot3d([box3d_p_tl[0], box3d_p_tr[0]], [box3d_p_tl[1], box3d_p_tr[1]],
    #                     [box3d_p_tl[2], box3d_p_tr[2]])
    #         mlab.plot3d([box3d_p_tr[0], box3d_p_dr[0]], [box3d_p_tr[1], box3d_p_dr[1]],
    #                     [box3d_p_tr[2], box3d_p_dr[2]])
    #         mlab.plot3d([box3d_p_dr[0], box3d_p_dl[0]], [box3d_p_dr[1], box3d_p_dl[1]],
    #                     [box3d_p_dr[2], box3d_p_dl[2]])
    #         mlab.plot3d([box3d_p_dl[0], box3d_p_tl[0]], [box3d_p_dl[1], box3d_p_tl[1]],
    #                     [box3d_p_dl[2], box3d_p_tl[2]])
    #     mlab.show(1)
    #     raw_input()
    ...
    #     # drwa 2d box in 3d
    #     # uvdepth [[ 2.16261821  0.63627729 10.        ]]
    #     distance = [1, 2, 3, 4, 5, 10, 15, 20, 30, 50, 70, 80, 100]
    #     # distance = [5]
    #     for d in distance:
    #         p_tl = np.array([[xmin, ymin, d]])
    #         p_tr = np.array([[xmax, ymin, d]])
    ...
    #         print('---p_dl', p_dl)
    #         print('---p_dr', p_dr)
    #         box3d_p_tl = calib.project_image_to_velo(p_tl)
    #         box3d_p_tr = calib.project_image_to_velo(p_tr)
    #         box3d_p_dl = calib.project_image_to_velo(p_dl)
    #         box3d_p_dr = calib.project_image_to_velo(p_dr)
    #         print('---box3d_p_tl', box3d_p_tl)
    #         print('---box3d_p_tr', box3d_p_tr)
    #         print('---box3d_p_dl', box3d_p_dl)
    #         print('---box3d_p_dr', box3d_p_dr)
    #
    #         mlab.plot3d([box3d_p_tl[0, 0], box3d_p_tr[0, 0]], [box3d_p_tl[0, 1], box3d_p_tr[0, 1]],
    #                     [box3d_p_tl[0, 2], box3d_p_tr[0, 2]])
    #         mlab.plot3d([box3d_p_tr[0, 0], box3d_p_dr[0, 0]], [box3d_p_tr[0, 1], box3d_p_dr[0, 1]],
    #                     [box3d_p_tr[0, 2], box3d_p_dr[0, 2]])
    #         mlab.plot3d([box3d_p_dr[0, 0], box3d_p_dl[0, 0]], [box3d_p_dr[0, 1], box3d_p_dl[0, 1]],
    #                     [box3d_p_dr[0, 2], box3d_p_dl[0, 2]])
    #         mlab.plot3d([box3d_p_dl[0, 0], box3d_p_tl[0, 0]], [box3d_p_dl[0, 1], box3d_p_tl[0, 1]],
    #                     [box3d_p_dl[0, 2], box3d_p_tl[0, 2]])
    #     mlab.show(1)
    #     raw_input()
    ...
    if show_lidar_frustum_with_3d:
        ...
        print('-boxfov_pc_velo', boxfov_pc_velo)
        print('-boxfov_pc_velo', boxfov_pc_velo.shape)
        print(('2d box FOV point num: ', boxfov_pc_velo.shape[0]))
        write_lidar_data(lidar_data='./utils/lidar_data{}.txt'.format(data_idx), data=boxfov_pc_velo)
...
if __name__=='__main__':
    ...
    parser.add_argument('--car_only', action='store_true', help='Only generate cars; otherwise cars, peds and cycs')  # default=False
    parser.add_argument('--with_image', action='store_true')
    args = parser.parse_args()

    print('---args.car_only', args.car_only)  # False
    if args.car_only:
        type_whitelist = ['Car']
    ...
    print('---args.with_image', args.with_image)  # False
    if args.with_image:
    ...
    print('---args.show_stats', args.show_stats)
    if args.show_stats:
        imagesets_file = os.path.join(BASE_DIR, 'image_sets/train.txt')
        print_box3d_statistics(imagesets_file, type_whitelist, 'train')
    print('---args.demo_object', args.demo_object)
    if args.demo_object:
    ...
    print('---args.gen_mini', args.gen_mini)
    if args.gen_mini:
    ...
    print('---args.gen_train', args.gen_train)
    if args.gen_train:
        print('Start gen_train...')
        imagesets_file = os.path.join(BASE_DIR, 'image_sets/train.txt')
        extract_frustum_data(\
    ...
    print('---args.gen_val', args.gen_val)
    if args.gen_val:
        print('Start gen_val...')
    ...
    print('---args.gen_val_rgb_detection', args.gen_val_rgb_detection)
