...
def get_lidar_in_image_fov(pc_velo, calib, xmin, ymin, xmax, ymax,
    ...
    # print('---pc_velo.shape', pc_velo.shape)   # (115384, 3)
    # print('---pc_velo', pc_velo)
    pts_2d = calib.project_velo_to_image(pc_velo)
    # print('---pts_2d.shape', pts_2d.shape)  # (115384, 3)
    # print('---pts_2d', pts_2d)
    fov_inds = (pts_2d[:,0]<xmax) & (pts_2d[:,0]>=xmin) & \
        (pts_2d[:,1]<ymax) & (pts_2d[:,1]>=ymin)
    # print('---fov_inds', fov_inds)
    # print('---fov_inds.shape', fov_inds.shape)   # (115384,)
...
def show_lidar_on_image(pc_velo, img, calib, img_width, img_height, showtime=False, grid_size=None):
    ...
    if grid_size is not None:
        cell_id = (int(img_width/grid_size), int((img_height/grid_size)))
        for id0 in range(1, cell_id[0]+1):
            cv2.line(img, (id0 * grid_size, 0), (id0 * grid_size, img_height), color=(255, 0, 0))
        for id1 in range(1, cell_id[1]+1):
            cv2.line(img, (0, id1 * grid_size), (img_width, id1 * grid_size), color=(255, 0, 0))
...
def show_lidar_with_2dbbox_on_image(im_name, pc_velo, bbox_2d, img, calib, img_width, img_height, masks=None, pc_add=None, showtime=False, pts_color=None, grid_size=None):
    ''' Project LiDAR points to image '''
    # print(pc_velo.shape)
    ...
    if pts_color:
        for i in range(imgfov_pts_2d.shape[0]):
            cv2.circle(img, (int(np.round(imgfov_pts_2d[i,0])),
                int(np.round(imgfov_pts_2d[i,1]))),
                1, color=pts_color, thickness=-1)
        p1, p2 = (bbox_2d[0], bbox_2d[1]), (bbox_2d[2], bbox_2d[3])
    ...
    if pc_add is not None:
        imgfov_pc_velo, pts_2d, fov_inds = get_lidar_in_image_fov(pc_add,
                                                                  calib, 0, 0, img_width, img_height, True)
        imgfov_pts_2d = pts_2d[fov_inds, :]
        for i in range(imgfov_pts_2d.shape[0]):
            cv2.circle(img, (int(np.round(imgfov_pts_2d[i, 0])),
    ...
    if masks is not None:
        for i in range(masks.shape[2]):
            img[:, :, 0] = np.where(masks[:, :, i] == 1, 255, img[:, :, 0])
