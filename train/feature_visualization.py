...
# from utils.show_lidar import show_lidar
...
def read_kfb_points_from_instance_pts_npz(instance_pts_npz):
    instance_pts_npz_name_full = os.path.join('./instance_pts_npz4', instance_pts_npz)
...
def decode_results(cls_maps, reg_maps, ref_maps, uv_maps, im_names, virtual_lidar_maps, real_lidar_maps):
    ...
    for i in range(batch_size):
        ...
        ref_map = ref_maps[i].detach().cpu().numpy()
        uv_map = uv_maps[i]
        im_name = im_names[i]
        virtual_lidar_map = virtual_lidar_maps[i].detach().cpu().numpy()   # torch.Size([1224, 370, 3])
        real_lidar_map = real_lidar_maps[i].detach().cpu().numpy()
        ...
        num = 0
        for u, v in uv_map_idx:
            # print(u, v)

            pc_virt = virtual_lidar_map[u, v, :]
            # print(pc_virt)
            # pc_virt[0] *= config.MAX_DISTANCE_X
            # pc_virt[1] *= config.MAX_DISTANCE_Y
            # pc_virt[2] *= config.MAX_DISTANCE_Z
            pc_real = real_lidar_map[u, v, :]
            # print(pc_real)
            # pc_real[0] *= config.MAX_DISTANCE_X
            # pc_real[1] *= config.MAX_DISTANCE_Y
            # pc_real[2] *= config.MAX_DISTANCE_Z
            ...
            if pc_cls == 'f':
                ...
                # print(pc_dl, 'f')
                # print(pc_pred, pc_virt, pc_real)
                if pc_dl < 0.6 and pc_dl > - 0.6:   # filter the big dl
                    pc_pred = get_predict_pc(dl=pc_dl, virtual_pc=pc_virt)
                    # print(pc_dl, 'f')
                    # print(pc_pred)
                    # exit()
        ...
        np.save(save_path + '/{}_real.npy'.format(im_name[:-4]), pcd_real_np)
        ...
        print('pcd_real, pcd_virt, pcd_pred: {}, {}, {}'.format(len(pcd_real), len(pcd_virt), len(pcd_pred)))
        print('---------------------------------------------------------------')
...
if __name__ == '__main__':
    ...
    # load data
    dataset_val = LinDataset(data_path=config.instance_val, label_path=config.label_path, test=True)
    dataloader_val = DataLoader(dataset_val, shuffle=False, batch_size=config.batch_size_test, num_workers=1)

    # model inference
    model.to(device)
    model.eval()
    for i, (datas, uv_map, virtual_lidar_maps, real_lidar_maps, im_names) in tqdm(enumerate(dataloader_val)):
        print(im_names[0])
        ...
        # decode_results(cls_maps=cls_map, reg_maps=reg_map, ref_maps=ref_map, uv_maps=uv_map, im_names=im_names,
        # virtual_lidar_maps=virtual_lidar_maps, real_lidar_maps=real_lidar_maps)
        ...
        print(np.unique(cls_map))

        print(cls_map.shape)
        cls_map = 255 * cls_map
        cls_map = cls_map.transpose()
        # plt.imshow(cls_map_f)
        plt.imsave('./hotmap/' + im_names[0], cls_map)
        if i == 2:
            exit()
